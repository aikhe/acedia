...
const pains = ref<{ id: number; text: string; done: boolean }[]>([
  {
    id: id++,
    text: 'this is acedia',
    done: false,
...
const filteredPains = computed(() => {
  return hideDone.value ? pains.value.filter((p) => !p.done) : pains.value
})

const addPain = () => {
  pains.value.push({ id: id++, text: newPain.value, done: false })
  newPain.value = ''

  pains.value.forEach((pain) => {
    console.log(`${pain.text} - ${pain.done}`)
  })
}
  ...
  <main>
    <div class="">
      <div>
        ...
        <ul v-for="(pain, index) in filteredPains" :key="index">
          <input type="checkbox" v-model="pain.done" />
          <span :class="{ done: pain.done }">
            {{ pain.id }}
            -
            {{ pain.text }}
          </span>
        ...
        <button @click="hideDone = !hideDone">
          {{ hideDone ? 'show done pain' : 'hide done pain' }}
        </button>
